...
app.get('/favs', function(req, res) {
  db.favs.find(function(err, contacts) {
    var favIds = contacts[0].favorites;
    console.log(favIds);
    res.json(favIds);
  });
});
...
app.delete('/contactList/:id', function(req, res) {
  ...
  console.log('removed contact id', id);
...
app.put('/contactList/:id', function(req, res) {
  ...
  db.contactList.findAndModify({query: {_id: mongojs.ObjectId(id)},
  	update: {$set: {name: req.body.name, email: req.body.email, number: req.body.number, nickname: req.body.nickname}},
...
app.post('/addToFavs/:id', function(req, res) {
  ...
  db.favs.findOne({
    ...
    var newFavList = favList.favorites;
    newFavList.push(id);
    db.favs.findAndModify({query: {name: 'favList'},
    update: {$set: {favorites: newFavList}},
    new: true}, function(err, updatedFavList) {
      if (err) {
        res.statusCode=500;
        res.json({});
      } else {
        res.statusCode=200;
        res.json(updatedFavList);
      }
    });
  })
